...
These require **real** ChatGPT/Codex and/or GitHub Copilot subscriptions and a TTY. They
are **not** part of the default suite (they carry the `live` marker and are skipped) and
...
uv sync --all-groups
uv run agw setup
uv run agw auth chatgpt      # and/or: uv run agw auth copilot
```
...
- [ ] GitHub Copilot main-session streamed response; proxy evidence shows the Copilot upstream.
